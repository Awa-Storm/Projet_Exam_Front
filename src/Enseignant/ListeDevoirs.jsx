...
function ListeDevoirs({ setView }) {
  ...
  useEffect(() => {
    const fetchExamens = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/examens/${idEnseignant}`);
        
        const examensWithCorrections = await Promise.all(
          response.data.map(async (examen) => {
            try {
              const correctionRes = await axios.get(`${API_URL}/api/correction-info/${examen.idExamen}`);
  ...
  const publierExamen = async (idExamen) => {
    try {
      await axios.put(`${API_URL}/api/examens/${idExamen}`, { publie: true });
  ...
  const consulterCorrection = async (idExamen) => {
    ...
    try {
      const response = await axios.get(`${API_URL}/api/correction/${idExamen}`);
  ...
  const telechargerCorrection = async (nomFichier) => {
    try {
      window.open(`${API_URL}/api/telecharger-correction/${nomFichier}`, '_blank');
    } catch (error) {
